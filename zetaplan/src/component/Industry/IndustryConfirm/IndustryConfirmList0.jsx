import React from 'react';
import './../../../css/Indusrty/industryConfirm.css';

const IndustryConfirmList0 = () => {
  return (
    <div id='IndustryConfirmList0'>
      <h4 className='company-road-part'>제타플랜 인증지원 차별점</h4>
      <p>제타플랜은 2006년부터 현재까지 국내 2,500여 개사의 기업 인증을 지원하고 있으며, 인증기업의 성장을 위한 <strong>기업금융</strong>, <strong>기술금융</strong>, <strong>투자유치</strong>, <strong>기술거래</strong>, <strong>M&A</strong>, <strong>해외 진출</strong> 등의 업무를 체계적으로 지원하여 고객사의 글로벌 기업의 성장을 위한 노력을 기울이고 있습니다.</p>
      <p>제타플랜은 <strong>이노비즈기업협회</strong>, <strong>벤처기업협회</strong>, <strong>경영혁신형중소기업협회</strong> 등의 <strong>3대 혁신형 기업</strong>을 지원하는 협회와 지원 업무를 공유하고 있으며, 녹색인증을 위한 녹색기술센터, 서울 산업진흥원, 경기테크노파크와도 협력하여 업무를 지원하고 있습니다.<br />그리고 한국발명진흥회, 신용보증기금, 한국산업은행, 한국 벤처캐피털협회, 중소기업진흥공단, 농업기술실용화 재단, 농수산식품유통공사, 한국 소재부품투자기관협의회, 국제기술교류재단, 한국산업기술진흥원 등과 협업하여 기업의 성공을 위한 지원 업무에 최선을 다하고 있습니다.</p>
      <p>지난 14년 동안 2,500개 이상의 기업이 왜 제타플랜을 선택했을까?</p>
      <p>작은 컨설팅이라도 제타플랜과 인연을 맺었다면, 당신은 자문조직을 얻으신겁니다.</p>
      <p>인증기업의 성장을 위한 컨설팅 체계적인 컨설팅 지원</p>
      <span>창업지원 / 기업금융 / IP금융 / 투자유치 / 해외진출 / 특허 기술거래 / IPO(코넥스, 코스닥) / M&A</span>
      <p>국내, 해외 정부컨설팅 지정 기관으로서 정부지원 컨설팅 사업 기회 제공</p>
      <span>&#183; 중소기업청 경영컨설팅 / 중소기업청 해외민간네트워크 사업 / 한국환경산업기술원 사업화기반 컨설팅 / 한국환경협회 컨설팅</span>
      <span>&#183; 소재부품투자기관협의회 투자유치 컨설팅 / 한국발명진흥회 상설 투자마트 투자지원 컨설팅 / 한국농수산식품유통공사 컨설팅</span>
      <span>&#183; 산업통상자원부 지정 기술거래기관으로서 기술이전 및 기술사업화 · 투자연계 R&D 컨설팅 지원 / KDB 산업은행 기술거래 연계지원</span>
      <span>&#183; 한국벤처캐피탈협회 M&A 지원사업, 신용보증기금 연계 보증연계투자· IPO · M&A 컨설팅</span>
      <h4 className='company-road-part'>제타플랜 인증 어떻게 다른가?</h4>
      <span>&#183; 인증 취득성공를 위한 사전 검토서비스 제공</span>
      <span>&#183; 2017년 해당 컨설팅 진행사 99% 인증 취득</span>
      <span>&#183; 인증로드맵 설계를 통한 향후 기업 성장 제공 멘토링 무료 제공</span>
      <span>&#183; 경영혁신 표준 메뉴얼 제공</span>
      <span>&#183; 제타플랜 컨설팅의 높은 인지도와 고객만족율</span>
      <span>&#183; 인증만료 안내 및 인증 변경관련 무료 상담 제공</span>
      <span>&#183; 전분야 상담가능한 고급컨설턴트의 1:1 업체 전담 배정 및 지원</span>
      <h4 className='company-road-part'>제타플랜 컨설팅 고객사</h4>
      <div className='industry-confirm0-img'><img src="/img/Industry/IndustryConfirm/confirm_info1.png" alt="" /></div>
      <p><strong>상기 기업 포함 2,500개사 인증 진행</strong></p>
    </div>
  );
};

export default IndustryConfirmList0;